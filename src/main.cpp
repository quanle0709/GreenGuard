...
#define GETNUM(k,field,type) if(in[k].is<type>())c.field=in[k].as<type>();else if(in[k].is<const char*>())c.field=(type)String(in[k].as<const char*>()).toInt()
 GETNUM("rainConfirmationMs",rainConfirmationMs,uint32_t);GETNUM("dryConfirmationMs",dryConfirmationMs,uint32_t);GETNUM("fullTravelTimeMs",fullTravelTimeMs,uint32_t);GETNUM("motorPwmSpeed",motorPwmSpeed,uint16_t);GETNUM("motorTimeoutMarginMs",motorTimeoutMarginMs,uint32_t);
#undef GETNUM
 if(!in["rainDigitalActiveLow"].isNull()) {
   c.rainDigitalActiveLow=in["rainDigitalActiveLow"].is<bool>()?in["rainDigitalActiveLow"].as<bool>():boolArg(in["rainDigitalActiveLow"]|"");
 }
