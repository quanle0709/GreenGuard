...
#include "config.h"
#include <math.h>
void MotorController::outputsOff(){analogWrite(PIN_MOTOR_RPWM,0);analogWrite(PIN_MOTOR_LPWM,0);digitalWrite(PIN_MOTOR_ENABLE,LOW);}
bool MotorController::begin(){pinMode(PIN_MOTOR_RPWM,OUTPUT);pinMode(PIN_MOTOR_LPWM,OUTPUT);pinMode(PIN_MOTOR_ENABLE,OUTPUT);outputsOff();analogWriteRange(1023);return true;}
...
void MotorController::activate(MotorState d){outputsOff();state_=d;lastDirection_=d;started_=millis();if(DEMO_MODE)return;digitalWrite(PIN_MOTOR_ENABLE,HIGH);bool physicalOpen=(d==MotorState::MOTOR_OPENING)^cfg_.motorDirectionReversed;analogWrite(physicalOpen?PIN_MOTOR_RPWM:PIN_MOTOR_LPWM,cfg_.motorPwmSpeed);}
